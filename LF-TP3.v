...
match Auto with
|Automate K Sigma delta s F => K
end.

(* "symboles" : prend en paramètre un automate et renvoie la liste des symboles de l'alphabet *)
Definition symboles (Auto : automate) : list Alphabet :=
match Auto with
|Automate K Sigma delta s F => Sigma
end.

(* "initial" : prend en paramètre un automate et renvoie l'état initial *)
Definition initial (Auto : automate) : nat :=
match Auto with
|Automate K Sigma delta s F => s
end.

(* "acceptant" : prend en paramètre un automate et un état q et renvoie true ssi q est un état final *)
Definition acceptant (Auto : automate)(q : nat) : bool :=
match Auto with
|Automate K Sigma delta s F => appartient q F
end.
...
Definition transition (Auto : automate) (q : nat) (c : Alphabet) : option nat :=
match Auto with
|Automate K Sigma delta s F => delta q c
end.
...
Definition M_nb_b_impair : automate :=
Automate (1::2::[]) (a::b::[]) delta_nb_b_impair 1 (2::[]).
...
Fixpoint execute (Auto : automate)(q : nat)(w : list Alphabet) : option nat :=
match Auto with 
|Automate K Sigma delta s F =>  match w with
                                |[] => Some q
                                |c::l => match delta q c with
                                         |None => None
                                         |Some n => execute Auto n l
                                         end
                                end
end.
...
Definition reconnait (Auto : automate) (w : list Alphabet) : bool :=
match Auto with 
|Automate K Sigma delta s F => match execute Auto s w with
                               |None => false
                               |Some n => acceptant Auto n
                               end
end.
